...
- [ ] Notion DB 실제 생성 (Trips + Places DB, 샘플 데이터 입력)
  - Trips DB: `Name`, `StartDate`, `EndDate`, `Status`(계획중/확정/완료), `CoverImage` 컬럼
  - Places DB: `Name`, `Category`(교통/숙소/맛집/명소), `Trip`(Relation → Trips DB), `VisitDate`, `Address`, `Latitude`, `Longitude`, `Time`, `ReservationNumber`, `Memo`, `Rating` 컬럼
- [ ] 환경변수 발급 및 설정 — `NOTION_API_KEY`, `NOTION_TRIPS_DB_ID`, `NOTION_PLACES_DB_ID`, `NEXT_PUBLIC_KAKAO_MAP_APP_KEY`
- [ ] Kakao Developers 앱 등록 — 웹 플랫폼에 `http://localhost:3000` 도메인 등록
- [ ] 신규 패키지 설치
  ```bash
...
- **Task 001: 환경 설정 및 신규 패키지 설치** - 우선순위
  - 신규 패키지 설치:
...
- **Task 005: 여행 목록 페이지 UI 완성 (더미 데이터)**
...
- **Task 006: 여행 대시보드 페이지 UI 완성 (더미 데이터)**
...
- **Task 008: Notion API 클라이언트 및 데이터 페칭 구현** - 우선순위
  - `lib/notion.ts` 완성:
...
- **Task 010: 여행 대시보드 페이지 Notion 연동 및 필터링 로직 구현**
...
- **Task 011: 지도 페이지 Notion 연동 및 마커 기능 구현**
  - `app/travel/[tripId]/map/page.tsx` 더미 데이터 교체
  - `MapView.tsx` Kakao Maps 마커 렌더링 완성:
    - `Place[]` 데이터의 `latitude`/`longitude`로 `kakao.maps.LatLng` 생성
    - `kakao.maps.CustomOverlay`로 카테고리별 색상 마커 표시 (`MARKER_COLORS` 상수 활용)
  - `MarkerPopup.tsx` 클릭 팝업 완성:
    - 마커 클릭 시 `kakao.maps.CustomOverlay` 팝업 표시
    - 팝업 내용: 장소명, 카테고리, 메모 표시
  - 지도 페이지 날짜 필터: 선택 날짜 기준 마커 필터링 (한국 시간 기준)
  - 전체 장소가 지도에 모두 보이도록 자동 중심/줌 설정
  - 위도/경도 누락된 장소는 마커 생성 제외 처리
  - Playwright MCP로 지도 페이지 E2E 테스트:
    - 마커 렌더링 확인 (카테고리별 색상 구분)
    - 마커 클릭 시 CustomOverlay 팝업 표시 확인
    - 날짜 필터 연동 확인
    - 모바일 전체 화면 / 데스크톱 사이드바 레이아웃 확인

- **Task 012: On-demand ISR 엔드포인트 및 새로고침 기능 구현**
  - `app/api/revalidate/route.ts` 완성:
    - `POST /api/revalidate` 엔드포인트 구현
    - `revalidatePath('/travel')`, `revalidatePath('/travel/[tripId]')` 호출
    - 간단한 요청 검증 (쿼리 파라미터 `tripId` 확인)
  - 대시보드 새로고침 버튼 기능 연동:
    - 클릭 시 `/api/revalidate` POST 요청 + `router.refresh()` 호출
    - 로딩 중 버튼 비활성화 및 스피너 표시
    - sonner 성공 토스트: "여행 정보가 업데이트되었습니다"
    - sonner 실패 토스트: "새로고침 실패. 잠시 후 다시 시도해주세요"
  - Playwright MCP로 ISR 및 새로고침 E2E 테스트:
    - 새로고침 버튼 클릭 후 성공 토스트 표시 확인
    - API 실패 시 에러 토스트 표시 확인
    - 데이터 갱신 후 화면 업데이트 확인

- **Task 013: 통합 테스트 및 전체 사용자 플로우 검증**
  - Playwright MCP를 활용한 전체 사용자 여정 E2E 테스트:
    - 홈 → 여행 플래너 메뉴 클릭 → 여행 목록 페이지 이동
    - 여행 목록 → 여행 카드 클릭 → 대시보드 이동
    - 대시보드 → 카테고리 탭 전환 → 날짜 필터 선택 → 카드 목록 필터링
    - 대시보드 → 지도 보기(모바일) / 사이드바 지도(데스크톱) 확인
    - 지도 → 마커 클릭 → CustomOverlay 팝업 → 뒤로가기 → 대시보드
    - 새로고침 버튼 동작 확인
  - 에러 케이스 테스트:
    - Notion API 실패 시 에러 화면 및 토스트 표시
    - 빈 여행 목록 / 빈 장소 목록 빈 상태 UI
    - 위도/경도 누락된 장소는 지도에서 제외 처리
  - 시간대 엣지 케이스 검증:
    - 자정 전후 D-Day 계산 정확성
    - 날짜 필터 한국 시간(UTC+9) 기준 정확성
    - Notion UTC 날짜 → 한국 시간 변환 오차 없음 확인
  - 반응형 레이아웃 검증: 모바일(375px), 태블릿(768px), 데스크톱(1280px)
