...
- ✅ **Task 019-A: PWA Manifest + 아이콘 + 레이아웃 메타태그** - 2026-03-19 완료
  ...
  - ✅ `public/icons/` 아이콘 파일 추가 (PNG 필수, SVG 불가)
    - `icon-192.png` (192×192), `icon-512.png` (512×512) — Node.js zlib deflate로 단색 PNG 생성
  - ✅ `app/layout.tsx` 수정 — `metadata` 객체에 PWA 필드 추가:
    - `appleWebApp: { capable: true, statusBarStyle: 'default', title: '여행 플래너' }`
    - `icons: { apple: '/icons/icon-192.png' }`

  **수락 기준:**
  - [x] `/manifest.webmanifest` GET 200 응답
  - [x] manifest 내 `name`, `icons`, `display: standalone` 포함
  - [x] `apple-mobile-web-app-capable` 메타태그 반영

- ✅ **Task 019-B: Service Worker 구현 및 등록** - 2026-03-19 완료
  ...
  - ✅ `providers/service-worker-register.tsx` 신규 생성 후 `app/layout.tsx`에 포함

  **수락 기준:**
  - [x] Chrome DevTools > Application > Service Workers에서 sw.js 등록 확인
  - [x] `/api/revalidate` POST 요청이 SW에 의해 차단되지 않음
  - [x] 정적 자산(JS/CSS) 캐시 스토리지에 저장 확인 (`static-v1`, `pages-v1` 캐시 생성)
  - [x] 네트워크 오프라인 전환 후 캐시된 페이지 로딩 확인

- ✅ **Task 019-C: 오프라인 폴백 페이지 + Playwright E2E 테스트** - 2026-03-19 완료
  - ✅ `app/offline/page.tsx` 신규 생성 (`"use client"`)
    - UI: MapPin 아이콘 + "오프라인 상태입니다" + 새로고침 버튼, 다크모드 지원
  ...
  **수락 기준:**
  - [x] `/offline` 페이지 렌더링 정상
  - [x] 네트워크 복구 시 자동으로 `/` 리다이렉트
  - [x] Playwright MCP 오프라인 시나리오 테스트 통과

  **테스트 체크리스트 (Playwright MCP):**
  - [x] `/manifest.webmanifest` GET 200 응답 확인
  - [x] `navigator.serviceWorker` 등록 확인 (scope: `/`, state: `activated`)
  - [x] `/offline` 폴백 페이지 렌더링 확인 (아이콘·제목·버튼 모두 정상)
  - [x] 정적 자산 캐시 스토리지 저장 확인 (`static-v1` 35개 항목, `pages-v1` 별도)
  - [x] 네트워크 복구 시 자동 리다이렉트 (online 이벤트 핸들러 등록 확인)

---

### Phase 6: 실시간 장소 관리

> 노션 앱 없이 웹 대시보드에서 직접 방문 장소를 추가할 수 있는 기능을 구현합니다.

- ✅ **Task 020: POST /api/places Route Handler 구현** - 2026-03-19 완료
  - `app/api/places/route.ts` 신규 생성
  - `@notionhq/client ^4.0.2` (실제 설치 버전)의 `notion.pages.create()` 호출로 Places DB에 새 장소 레코드 생성
  - Request body: `{ tripId, name, category?, visitDate?, visitDateEnd?, memo?, url?, cost? }`
  - **Zod 서버 검증 스키마** (PRD F022 기준):
    - `name`: `z.string().min(1).max(100)` — Notion title 최대 100자 제한
    - `category`: `z.enum(['교통','숙소','맛집','명소'])`, 기본값 `'명소'`
    - `visitDate`: `z.string().regex(/^\d{4}-\d{2}-\d{2}$/).optional()`
    - `visitDateEnd`: visitDate 없으면 사용 불가 (교차 검증)
    - `memo`: `z.string().max(2000).optional()` — Notion rich_text 최대 2000자
    - `url`: `z.string().url().optional()`
    - `cost`: `z.number().min(0).optional()` — 음수 방지
  - **CheckBox 기본값 `true`** — 방문 예정 상태로 즉시 카드 목록·지도에 표시
  - **relation 연결**: `trips: { relation: [{ id: tripId }] }` — 배열 형식 필수
  - 성공 시: 201 + `{ id, name }` 반환 + **`revalidatePath(\`/travel/${tripId}\`)` 리터럴 경로** 사용 (패턴 문자열 사용 시 무작동 주의)
  - 실패 처리: 400(검증 실패) / 401(인증 오류 — Integration 쓰기 권한 미설정 포함) / 404(tripId 없음) / 429(Rate Limit) / 500
  - **사전 조건**: Notion Integration의 Places DB **"Insert Content" 쓰기 권한 활성화** 필수
  - Playwright MCP E2E 테스트: 정상 생성 / 필수 필드 누락 오류 / 잘못된 tripId 오류

- ✅ **Task 021: AddPlaceDialog 컴포넌트 구현** - 2026-03-19 완료
  - `components/travel/AddPlaceDialog.tsx` 신규 생성
  - shadcn/ui `Dialog` + `Input` + `Select` + `Textarea` + `Label` 조합
  - **React Hook Form (`^7.x`) + Zod (`^4.x`) + `@hookform/resolvers`** — 모두 기존 프로젝트에 설치됨, 신규 의존성 없음
  - 필드 구성:
    - 장소명 (Input, 필수 — 레이블에 `*` 표시)
    - 카테고리 (Select: 교통/숙소/맛집/명소, 기본: 명소 — MARKER_COLORS 기반 색상 배지)
    - 방문 날짜 + 종료일 (Input type="date" 2열 grid — 날짜 범위 지원)
    - 메모 (Textarea, 선택)
    - 링크 (Input type="url", 선택)
    - 예상 비용 (Input type="number" min=0, 선택)
  - 저장 버튼 클릭 → `POST /api/places` 호출
  - 성공: **`router.refresh()` 호출** → 다이얼로그 닫기 + sonner 성공 토스트 "등록되었습니다"
    - `router.refresh()`는 서버 컴포넌트 재렌더를 트리거 — 기존 RefreshButton과 동일 패턴
    - TanStack Query `invalidateQueries` 미사용 (DashboardClient가 서버 props 기반이므로 캐시 대상 없음)
  - 실패: sonner 에러 토스트 "장소 등록에 실패했습니다" + 다이얼로그 유지
  - 로딩 중 버튼 disabled + Loader2 스피너 표시
  - 모바일 친화적 레이아웃 (`sm:max-w-[425px]`, 한 손 조작 가능)

- ✅ **Task 023: 장소 삭제 기능 구현 (F023)** - 2026-03-19 완료
  - `app/api/places/[placeId]/route.ts` 신규 생성 — DELETE Route Handler
  - `notion.pages.update({ page_id: placeId, archived: true })` — Notion은 영구 삭제 미지원, archived 처리
  - `revalidatePath(\`/travel/${tripId}\`)` — tripId 쿼리 파라미터로 전달받아 캐시 무효화
  - `components/travel/PlaceCard.tsx` 수정:
    - `"use client"` 추가 (onClick 핸들러 필요)
    - `DeleteButton` 서브컴포넌트 분리 (30줄 규칙 준수)
    - 우측 컬럼을 `flex flex-col` 세로 배치 — 외부링크(위) + 삭제버튼(아래)
    - 삭제 중 `Loader2` 스피너 + `pointer-events-none` 중복 요청 방지
  - `components/travel/DashboardClient.tsx` 수정: `<PlaceCard tripId={trip.id} />` prop 추가
  - 성공: `router.refresh()` + sonner "삭제되었습니다" 토스트
  - 실패: sonner "삭제에 실패했습니다" 토스트
  - Playwright E2E 테스트: 착한장칼국수 삭제 → 목록 6개, 맛집 3개, 합계 75,000원 확인

- ✅ **Task 024-A: PATCH /api/places/[placeId] Route Handler 구현 (F024)** - 2026-03-19 완료
  - `app/api/places/[placeId]/route.ts` 수정 — 기존 DELETE 핸들러에 PATCH 함수 추가
  - `patchPlaceSchema` 추가 (name 필수, 나머지 선택 — F022 Zod 스키마 재사용)
  - `buildPatchProperties(data)` 헬퍼 함수 분리 (30줄 규칙)
    - 입력값 존재 시만 해당 property 포함 (부분 업데이트)
    - visitDate 빈값: `{ date: null }` / URL 빈값: `{ url: null }` / Memo 빈값: `{ rich_text: [] }`
  - `notion.pages.update({ page_id: placeId, properties: buildPatchProperties(data) })`
  - tripId 쿼리 파라미터로 `revalidatePath(\`/travel/${tripId}\`)` 처리
  - 성공: 200 + `{ id, name }` / 실패: 400(검증) / 401(인증) / 429(Rate Limit) / 500

- ✅ **Task 024-B: EditPlaceDialog 컴포넌트 구현 (F024)** - 2026-03-19 완료
  - `components/travel/EditPlaceDialog.tsx` 신규 생성
  - props: `{ place: Place, tripId: string, open: boolean, onOpenChange: (open: boolean) => void }`
  - AddPlaceDialog의 서브컴포넌트 구조(FormFieldWrapper, CategoryOption, FormFields, makeSubmitHandler 팩토리) 동일 패턴으로 구현
  - React Hook Form `defaultValues`로 기존 장소 데이터 7개 필드 미리 채움
    - `costRaw: place.cost !== undefined ? String(place.cost) : ''`
  - DialogTrigger 없이 controlled Dialog 방식 (open/onOpenChange props로 외부 제어)
  - `PATCH /api/places/${place.id}?tripId=${tripId}` 호출
  - 성공: `router.refresh()` + `onOpenChange(false)` + sonner "수정되었습니다" 토스트
  - 실패: sonner "수정에 실패했습니다" 토스트 + 모달 유지
  - 로딩: 저장 버튼 `disabled` + `Loader2` 스피너 / 버튼 텍스트: "수정하기"

- ✅ **Task 024-C: PlaceCard 수정 버튼 추가 및 E2E 테스트 (F024)** - 2026-03-19 완료
  - `components/travel/PlaceCard.tsx` 수정
    - `EditButton` 서브컴포넌트 추가 (DeleteButton 패턴 참고, 30줄 규칙)
      - `isEditOpen` state + `EditPlaceDialog` 렌더링 포함
      - `Pencil` 아이콘 버튼, hover 시 `text-primary` 색상
    - 우측 액션 컬럼: 외부링크(상단) → `EditButton`(중간) → `DeleteButton`(하단)
    - `justify-between` 유지 (3개 요소 상단/중간/하단 자동 배치)
  - ✅ Playwright MCP E2E 테스트 통과:
    - 수정 버튼 클릭 → 모달 열림 + 기존 데이터 폼에 표시 확인
    - 장소명 변경 후 저장 → 카드 목록 즉시 반영 확인
    - 장소명 빈값 입력 시 저장 불가 확인
    - 취소 버튼 클릭 시 모달 닫힘 확인

- ✅ **Task 022: 대시보드 통합 및 E2E 테스트** - 2026-03-19 완료
  - ✅ `components/travel/DashboardClient.tsx` 수정:
    - 헤더 영역 우측 상단에 `+ 장소 추가` 버튼 추가 (Plus 아이콘)
    - `AddPlaceDialog` 컴포넌트 연동 (`tripId` prop 전달)
  - ✅ Playwright MCP E2E 테스트 통과:
    - 대시보드에서 "장소 추가" 버튼 클릭 → 모달 열림 확인
    - 필수 필드(장소명) 미입력 시 에러 표시 확인
    - 장소 저장 후 `router.refresh()` 동작 → 카드 목록 즉시 반영 확인
    - 저장된 장소 `checked=true`이므로 카드 목록에 즉시 노출 확인
...
| Notion 연동 | @notionhq/client ^4.0.2 | Trips DB, Places DB Relation 필터 조회/쓰기, `fetchWithRetry()` |
...
| F022 | 실시간 장소 추가 | Task 020, Task 021, Task 022 |
| F023 | 장소 삭제 | Task 023 |
| F024 | 장소 수정 | Task 024 |
